...
@Log4j2
@Service
@ServerEndpoint(value = "/socket/chatt/{idxState}")
public class WebSocketService { // 클라이언트가 접속할 때마다 생성되어 클라이언트와 직접 통신하는 클래스
    ...
    private ChatRepository cRepository = ApplicationContextProvider.ctx.getBean(ChatRepository.class);

    private static Set<Session> clients = Collections.synchronizedSet(new HashSet<Session>());
    List<Chat> chat;
    // LinkedHashMap<String,Object> hmap = new LinkedHashMap<String,Object>();
    List<Object> list = new ArrayList<Object>();
    List<Chat> clist = cRepository.findAll();
    ...
    // public List<Chat> getChat(Long tradeIdx) {
    // chat = cRepository.findByTradeIdx(tradeIdx);
    // return chat;
    // }
}